...
import { Chrome, Sparkles, UserRound } from 'lucide-react';
import formatFirebaseError from '../utils/formatFirebaseError';
import ElectricBackground from '../components/ElectricBackground';
import { Badge, Button, Card } from '../components/ui';
import { candidateAuth, candidateFirebaseReady } from '../config/candidateFirebase';
...
const CandidateSignupPage = () => {
  ...
  const handleGoogleSignup = async () => {
    if (!candidateFirebaseReady || !candidateAuth) {
      setError('Set the candidate Firebase environment variables before signing up.');
      return;
    }
  ...
  return (
    <div className="relative flex-1 min-h-0 overflow-hidden">
      ...
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_top,rgba(5,252,211,0.16),transparent_36%),linear-gradient(to_bottom,rgba(5,8,14,0.45),rgba(5,8,14,0.92))] pointer-events-none z-[1]" />
      ...
      <div className="relative z-10 flex items-center justify-center px-4 py-10 sm:py-14">
        <Card variant="elevated" padding="lg" className="w-full max-w-5xl overflow-hidden">
          <div className="grid gap-8 lg:grid-cols-[1.05fr_0.95fr] items-stretch">
            <div className="relative rounded-2xl border border-brand-border/40 bg-gradient-to-br from-brand-muted/20 via-transparent to-transparent p-6 sm:p-8 flex flex-col justify-between min-h-[260px]">
              <div>
                <Badge className="mb-4">Candidate Access</Badge>
                <h1 className="text-3xl sm:text-5xl font-extrabold text-white leading-tight">
                  Create your <span className="text-brand">Candidate Account</span>
                </h1>
                <p className="text-gray-300 text-sm sm:text-base mt-4 max-w-md leading-relaxed">
                  Join the candidate portal to manage your profile, prepare for interviews, and
                  explore AI-assisted hiring support.
                </p>
              </div>
            ...
            <div className="rounded-2xl border border-white/10 bg-surface/70 backdrop-blur-xl p-6 sm:p-8">
              ...
              <p className="text-gray-400 text-sm sm:text-base mt-2">
                This page uses the candidate Firebase app only.
              </p>

              {!candidateFirebaseReady && (
                <p className="mt-6 text-sm text-amber-300 bg-amber-500/10 border border-amber-500/30 rounded-xl px-4 py-3">
                  Configure the candidate Firebase env vars in frontend/.env before testing signup.
                </p>
              )}
              ...
              <button
                ...
                disabled={!candidateFirebaseReady || loading}
              ...
              <form className="space-y-5" onSubmit={handleSubmit}>
                ...
                <Button
                  ...
                  type="submit"
                  loading={loading}
                  disabled={!candidateFirebaseReady}
                >
