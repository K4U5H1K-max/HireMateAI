...
const CandidateLoginPage = () => {
  ...
  const handleGoogleLogin = async () => {
    if (!candidateFirebaseReady || !candidateAuth) {
      setError('Set the candidate Firebase environment variables before logging in.');
      return;
    }
            ...
            <div className="relative rounded-2xl border border-brand-border/40 bg-gradient-to-br from-brand-muted/20 via-transparent to-transparent p-6 sm:p-8 flex flex-col justify-between min-h-[260px]">
              <div>
                ...
                <h1 className="text-3xl sm:text-5xl font-extrabold text-white leading-tight">
                  Candidate <span className="text-brand">Login</span>
                </h1>
                <p className="text-gray-300 text-sm sm:text-base mt-4 max-w-md leading-relaxed">
                  Continue with your candidate account to access interview prep, resume analysis,
            ...
            <div className="rounded-2xl border border-white/10 bg-surface/70 backdrop-blur-xl p-6 sm:p-8">
              ...
              {!candidateFirebaseReady && (
                <p className="mt-6 text-sm text-amber-300 bg-amber-500/10 border border-amber-500/30 rounded-xl px-4 py-3">
                  Configure the candidate Firebase env vars in frontend/.env before testing login.
                </p>
              )}
